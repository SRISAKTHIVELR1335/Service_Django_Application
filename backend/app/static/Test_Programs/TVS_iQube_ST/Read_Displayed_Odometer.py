...
def Read_Displayed_Odometer():
    ...
    try:
        ...
        request = can.Message(
            ...
            data=[0x03, 0x22, 0xD1, 0x22, 0x00, 0x00, 0x00, 0x00],
        ...
        while time.time() - start_time < 2.0:
            ...
            if response:
                ...
                # Check for positive response 62 D1 22 at ID 0x7F1
                if (
                    response.arbitration_id == 0x7F1
                    and len(response.data) >= 5
                    and response.data[1] == 0x62
                    and response.data[2] == 0xD1
                    and response.data[3] == 0x22
                ):
                    # scaling: 0.1 km per bit
                    odometer = response.data[4] * 0.1
                    formatted = f"{odometer:.1f} km"
                    print("Odometer Reading:", formatted)
