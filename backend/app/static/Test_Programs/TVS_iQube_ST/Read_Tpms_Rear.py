...
def Read_Tpms_Rear(expected_mac, interface="PCAN_USBBUS1", bitrate=500000, timeout=5):
    print("TEST SEQUENCE : READ_TPMS_REAR")
    ...
    try:
        ...
        while True:
            ...
            if actual_mac:
                ...
                print(f"READ_TPMS_REAR: {status} - Read MAC: {actual_mac}, Expected: {expected_mac}")
                return (success, actual_mac.upper())  # Tuple: match success, actual MAC

        print("READ_TPMS_REAR: No valid response or MAC not found within timeout.")
        return (False, None)
...
if __name__ == "__main__":
    Read_Tpms_Rear("C0368754DDDD", timeout=5)